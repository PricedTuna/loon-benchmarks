...
export function generateEmployees(count: number): { employees: Employee[] } {
  return {
...
export function generateOrders(count: number): { orders: Order[] } {
  return {
 ...
 *
 * @remarks
 * The original suite included a "structural validation" track with five
 * deliberately corrupted variants (truncated, extra rows, width mismatch,
 * missing fields). Those datasets only have a meaningful answer for formats
 * that expose explicit length/schema metadata; for JSON/YAML the model has
 * to infer schema by counting. They were measuring metadata exposure, not
 * retrieval quality, and have been removed from the headline metric. The
 * fixture generator above is kept because the roundtrip-fidelity benchmark
 * uses it as a corruption-detection probe in a clearly separate report.
 */
...
export { generateStructuralValidationFixtures }
