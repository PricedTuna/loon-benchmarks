 ...
 *
 * @remarks
 * Only content-bearing question types are kept. The original suite shipped
 * `structure-awareness` and `structural-validation` categories whose answers
 * (record count, field-name list, ordinal field name, last-row lookup,
 * "is the array truncated") are directly readable from formats that expose
 * length markers and explicit field headers, and require manual counting in
 * formats that do not. Including them is equivalent to scoring formats on
 * how much format-native metadata they expose, which is not the property
 * this benchmark is meant to measure. They have been removed.
 */
